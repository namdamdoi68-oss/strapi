...
describe('Filters', () => {
  ...
  describe('duplicate filters (EE-75)', () => {
    ...
    it('should still edit the intended filter after another chip is removed mid-edit', async () => {
      const { user } = render();
      ...
      // Open the edit popover on `modified` (index 1)…
      await user.click(screen.getByText('Status $eq modified'));

      await waitFor(() => {
        expect(screen.getByRole('button', { name: 'Update filter' })).toBeInTheDocument();
      });

      // …then remove `draft` (index 0) without closing the popover, shifting everything down.
      fireEvent.click(screen.getByRole('button', { name: 'Status $eq draft' }));
      ...
      // Submitting must still rewrite `modified` — the entry the form is showing —
      // and must not touch `published`, which now occupies the index `modified` used to have.
      await user.click(await screen.findByRole('combobox', { name: 'Status' }));
      ...
      await waitFor(() => {
        expect(getAppliedFilterEntries()).toEqual([
          'filters[$and][0][status][$eq]=draft',
          'filters[$and][1][status][$eq]=published',
        ]);
      });
      expect(screen.queryByText('Status $eq modified')).not.toBeInTheDocument();
    });
